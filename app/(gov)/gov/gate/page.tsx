...
export default async function GatePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    ...
    >
      <div className="rounded-lg border border-amber-400/40 bg-amber-500/15 p-4 text-sm text-amber-800 dark:text-amber-200">
                    ...
                    <ol className="mt-1 space-y-1 text-sm">
                      {r.shortlist.map((s) => (
                        <li key={s.rank} className="rounded-md border border-border p-2">
                          ...
                          <span className={`ml-2 rounded px-1.5 py-0.5 text-[11px] ${s.notified ? "bg-emerald-500/15 text-emerald-800 dark:text-emerald-200" : "bg-neutral-500/15 text-neutral-300"}`}>
